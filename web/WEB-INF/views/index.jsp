                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        for(var i=0;i<data.length;i++){
                            refreshHtml +='<dl>'+
                            '<dt><a href="<%=request.getContextPath()%>/chart/chart?merchandiseid='+ data[i].merchandiseid+'" target="_new">'+
                            '<img src="<%=request.getContextPath()%>/images/T1.jpg" width="310" height="310" border="0" /></a>'+
                            '</dt>'+
                            '<dd>' +data[i].merchandisename +data[i].spec + data[i].name + '</dd>'+
...
<div id="box">
    <!--top start -->
    <div id="top">
        <a href="/main/index"><img src="<%=request.getContextPath()%>/images/logo.gif" alt="Estimation" width="255" height="58" border="0" class="logo" /></a>
        <p class="topDiv"></p>
        <p class="navLeft"></p>
        <ul>
            <li><a href="/main/index" class="hover">首页</a></li>
            <li><a href="#">关于我们</a></li>
            <li><a href="#">在线客服</a></li>
...
ssssssssss<br />
        ...
        <div id="mid">
            ...
            <div class="hotsale" id="productContent">
                <%
                    List<TmeMerchandiseinfo> merchandiselist=(List<TmeMerchandiseinfo>)request.getSession().getAttribute("merlist");
                %>

                <c:forEach items="<%=merchandiselist%>" var="merchandise">
                    <dl>
                        <dt><a href="<%=request.getContextPath()%>/chart/chart?merchandiseid=${merchandise.merchandiseid}" target="_new"><img src="<%=request.getContextPath()%>/images/T1.jpg" width="310" height="310" border="0" /></a></dt>
                        <dd>${merchandise.merchandisename} ${merchandise.spec} ${merchandise.name}</dd>
                        <dd><span class="viv1">￥:${merchandise.price}</span>
                            <span class="viv2"><a class="addchart" merid="${merchandise.merchandiseid}" target="_new"><img src="<%=request.getContextPath()%>/images/vivioow_b2.jpg" width="80" height="24" border="0"/></a></span>
                        </dd>
                    </dl>
                </c:forEach>
            ...
            <div class="pagnation" id="pagnation" >
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+-2%> class="page-prev">上一页</a>
                <a href=# class="current"><%=limit%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit%>><%=limit+1%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+1%>><%=limit+2%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+2%>><%=limit+3%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+3%>><%=limit+4%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+4%>>...</a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+11%>><%=limit+12%></a>
                <a href=<%=request.getContextPath()%>/main/page?limit=<%=limit+1%> class="page-next">下一页</a>
            </div>
